	...
	try{
		xhr.onreadystatechange = function(){
			if(this.readyState == 4 && this.status == 200) {
				...
				if(userId < 1){
					document.getElementById("loginResult").style.backgroundColor = "red";
					document.getElementById("loginResult").style.padding = "4px 15px 4px 15px";
					document.getElementById("loginResult").style.border = "1px solid white";
		...
		xhr.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
				...
				for(let i = 0; i < jsonObject.results.length; i++) {
					...
					//link.onClick = document.getElementById("searchResult").innerHTML = "You clicked it...";//function() {
					link.innerHTML = "Open";
          			link.style.backgroundColor = "#31CCB2";
					link.style.color = "white";
...
function doRegister(){
	...
	{
    document.getElementById("registerResult").innerHTML = "First Name Must Be Filled";
		return;
	}
	let lastNameRegister = document.getElementById( "registerlastName" ).value;
	if(lastNameRegister == 0)
	{
    document.getElementById("registerResult").innerHTML = "Last Name Must Be Filled";
		return;
	}
			...
			if (this.readyState == 4 && this.status == 200)
			{
				...
				if(jsonObject.error == "Login Already Exists")
				{
					...
					return;
						...
						window.location.href = "index.html";
            			sleepFor(500);
            			document.getElementById("accountCreated").innerHTML = "Account Successfully Created!";
					}
			}
		};
		xhr.send(jsonPayload);
...
function doAdd(){
  ...
  let firstNameAdd = document.getElementById( "addFirstName" ).value;
	...
	if(emailAdd == 0 || !isEmail(emailAdd))
	{
		document.getElementById("addContactResult").style.color = "red";
		document.getElementById("addContactResult").style.backgroundColor = "#EBC4C4";
		document.getElementById("addContactResult").style.display = "block";
	...
	if (phoneAdd.length != 10 || !isPhone(phoneAdd))
	{
		document.getElementById("addContactResult").style.color = "red";
		document.getElementById("addContactResult").style.backgroundColor = "#EBC4C4";
		document.getElementById("addContactResult").style.display = "block";
	...
	let tmp = {UserID:userId,FirstName:firstNameAdd,LastName:lastNameAdd, Email: emailAdd, Phone: formattedPhoneNumber};
		...
		xhr.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				...
				if(jsonObject.error == "Contact Already Exists"){
					...
					document.getElementById("addContactResult").style.display = "block";
					document.getElementById("addContactResult").innerHTML = "new contact added";
					window.location.href = "contacts.html";
        		}
			}
		};
...
function startDelete()
{
  ...
  if (confirmDelete)
  {
	...
	document.getElementById("displayResult").style.display = "block";
	document.getElementById("displayResult").innerHTML = "contact deleted successfully";
    doSearch();
  }
  else
  {
